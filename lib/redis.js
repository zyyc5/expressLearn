...
let config = require('../lib/config/config').redisServer;
 ...
 * 15: blacklist
 * 14: accessToken
 * 13: identifyCode 短信验证码
 * 12: userConfig
 * 11: visitCount || repeatVisit ip限制 和 重复访问
 * 10: 其它信息 (首页热搜等等) earnPoints
 * 9: companyCategory  公司类别
 * 8: 记录用户访问某些接口的次数. e.g., 老板详情页面, 普通用户每天只能访问3次
 * 
 */

class RedisPool {

  constructor({port, host, redisSecret, defaultDBIndex}) {
    let create = () => {
    ...
    this.pool = genericPool.createPool({create: create, destroy: destroy}, opts);
  ...
  static connect({
    ...
    return new this({port: port, host: host, redisSecret: redisSecret, defaultDBIndex});
  }
  ...
  [_invalidParamsError](paramsMap) {
    ...
    return {error: 'Invalid redis params', message: message};
  }
          ...
          if (paramsMap.has(symbol)) {
            ...
            if (mapValue === undefined) {
              reject(this[_invalidParamsError](
                new Map([[Symbol.keyFor(symbol), mapValue]])
              ));
            }
          }
          resolve(true);
   ...
   */
  putValue({key, value, seconds, dbIndex = this.defaultDBIndex} = {}) {
    let validParams = () => this[_validParams](new Map([[KeyKey, key], [KeyValue, value]]));
    let getClient = () => this[_getClient](dbIndex);
    let doRedisSet = cli => {
   ...
   */
  delKey({key, dbIndex = this.defaultDBIndex}) {
    let validParams = () => this[_validParams](new Map([[KeyKey, key]]));
    let getClient = () => this[_getClient](dbIndex);
    let doRedisDel = cli =>
   ...
   */
  getTtl({key, dbIndex = this.defaultDBIndex}) {
    let validParams = () => this[_validParams](new Map([[KeyKey, key]]));
    let getClient = () => this[_getClient](dbIndex);
   ...
   */
  listPush({key, value, seconds, dbIndex = this.defaultDBIndex}) {
    let validParams = () => this[_validParams](new Map([[KeyKey, key], [KeyValue, value]]));
    let getClient = () => this[_getClient](dbIndex);
    let doRedisLpush = cli =>
      cli.lpushAsync(key, value).then(length =>
        ({reply: length, client: cli}));
    let doRedisExpire = ({reply, client}) => {
   ...
   */
  listAll({key, dbIndex = this.defaultDBIndex}) {
    let validParams = () => this[_validParams](new Map([[KeyKey, key]]));
    let getClient = () => this[_getClient](dbIndex);
    let doLRange = cli =>
   ...
   */
  exec({cmd, argv, dbIndex = this.defaultDBIndex}) {
    let getClient = () => this[_getClient](dbIndex);
    let exec = cli => {
   ...
   */
  multi({cmds, dbIndex = this.defaultDBIndex}) {

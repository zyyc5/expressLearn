...
var _redisCONTANCe = redis.connect();
...
let util = {
  ...
  get redis() {
    if (!_redisCONTANCe)
      _redisCONTANCe = redis.connect()
    return _redisCONTANCe;
  }
};
...
util.queryTranstions = (querys, schemasNo = 1) => {
  ...
  return getTransConnect(writeAbleConnect, readAbleConnect).then(({
    wConn,
    rConn
  }) => {
    //有序执行
    if (Array.isArray(querys))
      return Promise.mapSeries(querys, (query) => {
        ...
        if (Array.isArray(query))
          [sql, values] = query;
        return choseConnect(sql, wConn, rConn).queryAsync({
          sql: sql,
          values: values
        });
      }).then(result => {
    ...
    for (let p in querys) {
      ...
      if (Array.isArray(querys[p]))
        [sql, values] = querys[p];
      queryPromise[p] = choseConnect(sql, wConn, rConn).queryAsync({
        sql: sql,
        values: values
      });
    }
